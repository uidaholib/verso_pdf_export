...
def download_asset_files(final_output):
    ...
    logger.info(
        f"File download complete: {total_files} files downloaded to 'A', {len(errors)} errors"
    )
...
def generate_metadata_csv(
    ...
    """
    Generate a CSV in the output directory containing metadata for each downloaded PDF.
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    """Parse CLI arguments for the VERSO PDF export script."""
    parser = argparse.ArgumentParser(
        description="Fetch metadata and download PDFs from VERSO/Esploro."
    )
    parser.add_argument(
        "--csv",
...
def main(argv: list[str] | None = None) -> None:
    """Orchestrate the full VERSO PDF export pipeline."""
